...
if __name__ == "__main__":
    ...
    D_max_1000=numpy.zeros((10,15))
    D_max_Nobs=numpy.zeros((10,15))
    ...
    fig = plt.figure()
    plt.set_cmap('jet_r')
    x_axis = [r'$\stackrel{%.2f}{%.2f}$'%(freqs[x]/1e9,kas[x]) for x in range(15)]
    ...
    plt.title("$N_{dipoles}=%d$, a=%.1f, $N_{MC}=%d$, $R=%d m$"%(N_dipole,a_EUT,N_MC,distance))
    #plt.show()
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.3/result.pdf')
