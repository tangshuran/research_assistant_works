...
if __name__ == "__main__":
    ...
    fig = plt.figure()
    plt.set_cmap('jet_r')
    x_axis = [int(N_dipole) for N_dipole in numpy.linspace(3,30,10)]
    ...
    plt.title("$a_{EUT}=%.4f$, f=%d MHz, $N_{MC}=%d$, $R=%d m$"%(a_EUT,f/1e6,N_MC,distance))
    #plt.show()
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.6/result_a.pdf')
