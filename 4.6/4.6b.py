...
if __name__ == "__main__":
    ...
    f = 1000*1e6
    a_EUT=2.0# radius of EUT
    N_dipole_list=numpy.linspace(3,30,10)   # number of random dipoles
    N_obs_points_list=numpy.append(numpy.array(range(10,160,10)),1000)
    ...
    D_max_1000=numpy.zeros((15,10))
    D_max_Nobs=numpy.zeros((15,10))
    ...
    fig = plt.figure()
    plt.set_cmap('jet_r')
    x_axis = [N_dipole for N_dipole in numpy.linspace(3,30,10)]
    y_axis = [N_obs for N_obs in numpy.array(range(10,160,10))]
    ax = fig.add_subplot(111)
    cax = ax.matshow(D_max_ratio)
    ax.xaxis.set_ticks_position('bottom')
    ax.set_xticks(numpy.arange(10))
    ax.set_yticks(numpy.arange(15))
    ...
    plt.title("$a=%d$, f=%d MHz, $N_{MC}=%d$, $R=%d m$"%(a_EUT,1000,N_MC,distance))
   # plt.show()
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.6/result_b.pdf')
