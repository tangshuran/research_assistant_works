...
if __name__ == "__main__":
   ...
   #hpoints=int(round(float(N_obs)/upoints))
    ...
    #N_obs=None
    N_obs_points_list=[[N_w,100] for N_w in numpy.array(range(5,100,5))] #number of observation points [Winkel,Höhe]
    #print N_obs_points
    ...
    for N_obs_points in N_obs_points_list:
        for f,ka,clr in zip(freqs,kas,colors):   # loop frequencies, kas
            ...
            for mc in range(N_MC): # MC loop
                ...
                #print mc, ma, av, D
                Ds.append(D)
            if N_obs_points[0]==100:
                D_max_100[:,j]=numpy.mean(Ds)
                j +=1
            else:
                D_max_N_w[i,j]=numpy.mean(Ds)
                j +=1
        ...
        i +=1
        j=0
    D_max_ratio=numpy.divide(D_max_N_w,D_max_100)
    fig = plt.figure()
    plt.set_cmap('jet_r')
