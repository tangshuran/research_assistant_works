...
if __name__ == "__main__":
    ...
    freqs=numpy.array(numpy.linspace(1000,6000,51))*1e6#freqs=numpy.array([1000000000])#,1000000000,1500000000])#range(30,301,30))*1000000#numpy.logspace(10,11,3)  # generate frequencies
     # vector with k*a values (a: EUT radius)
    ...
    for a_EUT,clr in zip(a_EUT_list,colors):
        y_value_list=[]
        kas=a_EUT*2*pi*freqs/c
        for f,ka in zip(freqs,kas):
            ...
            for mc in range(N_MC): # MC loop
                ...
                av=sum(Emags2)/N_obs # the average of |E|**2
                ma=max(Emags2) # the maximum of |E|**2
                D=ma/av # directivity
                #print mc
                Ds_R.append(D)
                Es2_max_list_R.append(ma)
            ...
            y_value=4*np.mean(Ds_K)/(np.mean(Es2_max_list_R)/np.mean(Es2_av_list_K))  
            y_value_list.append(y_value)
            #print    
            #print f, distance,deltah
            #print
        plt.plot(freqs/1e9,y_value_list, '%s+-'%clr,label="a=%.2fm"%(a_EUT))
        output_data.append(y_value_list)

    plt.legend(loc=4)
    plt.xlabel(r'$f/GHz$',fontsize=20,labelpad=10)
    plt.ylabel(r"$\frac{4<D^{K}_{max}>}{<E^{R^{2}}_{max}/E^{K^{2}}>}$")
    plt.title("$N_{dipoles}=%d$, MC runs=%d, R=%d m$"%(N_dipole,N_MC,distance))
    plt.grid()
    #pylab.show()
    #numpy.save(r"D:\wang\python-script\new_new_results\4.10/Ds_list",data)
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.11/result_b.pdf')
    pylab.savefig(pp, format='pdf',dpi=fig.dpi, bbox_inches='tight')
    pp.close()
    output=zip(*output_data)
    numpy.savetxt(r"D:\HIWI\python-script\new_new_results\4.11/4.11b.dat", output, fmt=['%.6f']*len(output_data))