...
if __name__ == "__main__":
    ...
    f = 1000*1e6   # radius of EUT
    N_dipole = 10    # number of random dipoles
    N_obs_points_list=numpy.append(numpy.array(range(10,110,10)),1000)
    N_MC=1000     # number of MC runs -> average over different random configurations
    a_EUT_list=numpy.array(numpy.linspace(0.1,1.9,10))#[30,50,80,100,150, 200,250, 300,350, 400,450, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500])*1e6#numpy.array(range(30,301,30))*1000000#numpy.logspace(10,11,3)  # generate frequencies
    kas=a_EUT_list*2*pi*f/c # vector with k*a values (a: EUT radius)
    deval=numpy.linspace(1,6,100)
    Ns=Ns_hansen_1D
    n_listen=0
    i=0
    j=0
           
    D_max_1000=numpy.zeros((10,10))
    D_max_Nobs=numpy.zeros((10,10))
    ...
    for N_obs_points in N_obs_points_list:
        for a_EUT,ka,clr in zip(a_EUT_list,kas,colors):   # loop frequencies, kas
            ...
            for mc in range(N_MC): # MC loop
                ...
                Ds.append(D)
                Eslist.append(Es)
            if N_obs_points==1000:
                D_max_1000[:,j]=numpy.mean(Ds)
                j +=1
            else:
                D_max_Nobs[i,j]=numpy.mean(Ds)
                j +=1
            print N_obs_points
    ...
    fig = plt.figure()
    plt.set_cmap('jet_r')
    x_axis = [r'$\stackrel{%.2f}{%.1f}$'%(a_EUT_list[x],kas[x]) for x in range(10)]
    y_axis = [N_obs for N_obs in numpy.array(range(10,110,10))]
    ...
    plt.xlabel(r'$\stackrel{a_{EUT}}{ka}$')
    plt.ylabel('$N_{obs}^{R}$')
    cb=fig.colorbar(cax)
    cb.set_label(r'$\frac{D_{max}^{R,N_{obs}}}{D_{max}^{R,1000}}$')
    plt.title("$N_{dipoles}=%d$, f=%d MHz, $N_{MC}=%d$, $R=%d m$"%(N_dipole,f/1e6,N_MC,distance))
    #plt.show()
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.5/result.pdf')
