...
if __name__ == "__main__":
    ...
    N_obs_points_list=numpy.arange(100,2000,100)#number of observation points (randomly distributed) on Ring around EUT
    N_MC=1000        # number of MC runs -> average over different random configurations
    freqs=numpy.array(numpy.linspace(100,1500,16))*1e6#freqs=numpy.array([30,50,100, 200, 400, 600, 700, 800, 900, 1100, 1200, 1300, 1400])*1000000#numpy.array(range(30,301,30))*1000000#numpy.logspace(10,11,3)  # generate frequencies
    kas=a_EUT*2*pi*freqs/c # vector with k*a values (a: EUT radius)
    deval=numpy.linspace(15,45,100)
    Ns=Ns_hansen_1D
    n_listen=0
    fig=pylab.figure()                
    ...
    for N_obs_points in N_obs_points_list:
        for f,ka,clr in zip(freqs,kas,colors):   # loop frequencies, kas
            ...
            pylab.plot(deval,ecdfD(deval), '%s+-'%clr, label="ECDF (Dipoles), f=%.2f MHz, N_obs=%s"%(f*1e-6,N_obs_points))
