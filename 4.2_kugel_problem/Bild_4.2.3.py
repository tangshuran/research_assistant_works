...
def calc_ds (sources, Nrepetitions,k,ophis,othetas):
    ...
    for rep in range(Nrepetitions):
        ...
        for ph,th in zip(ophis,othetas):
            ps=s.calc_power_ff(k,th,ph)  # using the function inside the Class
...
def R_Kugel(N,distance,zoffset=0):
    ...
    for k in (numpy.array(range(N-2))+2):
            ...
            phi=phiminus1+3.6/sqrt(N)*1/sqrt(1-hk**2)
            #print phi
            phiminus1=phi
            #thetaphilist.append([phi,theta])
            thetas.append(theta)
            phis.append(phi)
            thetaphilist.append([0,pi])    
            thetas.append(pi)
            phis.append(0)
        ...
        #print thetas,phis
            xyz=(numpy.array([distance*numpy.sin(thetas)*numpy.cos(phis),
                              distance*numpy.sin(thetas)*numpy.sin(phis),
                          distance*numpy.cos(thetas)+zoffset])).T
        #print xyz
...
if __name__ == "__main__":
    ...
    N_obs_points=500 #number of observation points (randomly distributed) on Ring around EUT
    N_MC=1000        # number of MC runs -> average over different random configurations
    freqs=numpy.array([30,50,80,100,150,200,250,300,350,400,450,500,600,700,800,900,1000,1100,1200,1300,1400,1500])*1e6#freqs=numpy.array([30,50,100, 200, 400, 600, 700, 800, 900, 1100, 1200, 1300, 1400])*1000000#numpy.array(range(30,301,30))*1000000#numpy.logspace(10,11,3)  # generate frequencies
    ...
    for f,ka,clr in zip(freqs,kas,colors):   # loop frequencies, kas
        ...
        [rs,phis,thetas]=R_Kugel(N_obs_points, distance)#, theta=0.5*pi,zoffset=0)#1.5) # generate not random observation points 
        #print 'rs',type(rs)
        ...
        pylab.figure(1)
        pylab.plot(deval,ecdfD(deval), '%s+-'%clr, label="ECDF (Dipoles), f=%.2f MHz"%(f*1e-6))
        #pylab.plot(deval,ecdfMa(deval), '%so-'%clr, label="ECDF (Ma), f=%.2f MHz"%(f*1e-6))
        #pylab.plot(deval,ecdfMCH(deval), '%ss-'%clr, label="ECDF (MC Hansen), f=%.2f MHz"%(f*1e-6))
        #pylab.plot(deval,[FD_hansen(d,ka,mu=1.,Ns=Ns) for d in deval],'%s-'%clr, label="Chi2,2 power Ns")
        #pylab.plot(ED,FD_hansen(ED,ka,mu=1.,Ns=Ns),'%s+'%clr,label='')
        xachs=deval
        yachs=ecdfD(deval)
        funcmap=0
        funcmap=dict(progtype='python',modus='Kugel',N_obs=N_obs_points, N_dipole=N_dipole ,Ds=Ds, euttype='Kugel',f=f ,N_MC=N_MC,xachs=xachs,yachs=yachs,a=a_EUT,R=distance,ka=ka)
        print funcmap
        name="daten/kugel/Kugel_{:.0f}dp_{:.0f}mc_{}Nobs_{}a_{}R_{:.0f}MHz.dmp".format(N_dipole,N_MC,N_obs_points,a_EUT,distance,f*1e-6)
        print name
    ...
    pylab.ylabel("CDF")
    pylab.title("$N_{dipoles}=%d$, a=%.1f, MC runs=%d,  $R=%d m$"%(N_dipole,a_EUT,N_MC,distance))
    pylab.show()
