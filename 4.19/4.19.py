...
if __name__ == "__main__":
    ...
    n_listen=0
    fig1=pylab.figure(1)  
    output_data1.append(N_obs_points_list) 
    output_data2.append(N_obs_points_list)     
    ...
    for f,ka,clr in zip(freqs,kas,colors):
        ...
        pylab.figure(2)
        pylab.plot(N_obs_points_list,D_max_var_list, '%s+-'%clr, label="f=%dGHz"%(f/1e9))
        output_data2.append(D_max_var_list)
    pylab.figure(1)
    pylab.axis([0,N_obs_points_list[-1],1,4])
    pylab.grid()
    pylab.xticks(list(pylab.xticks()[0]) + [1])
    pylab.legend(loc=4)
    pylab.xlabel("$N_{dipoles}$")
    pylab.ylabel("$<D_{max}^{R}>$")
    pylab.title(" MC runs=%d, $a_{EUT}=%.4fm$, $R=%dm$,$N_{dipole}=%d$"%(N_MC,a_EUT,distance,N_dipole))
    pylab.figure(2)
    pylab.axis([0,N_obs_points_list[-1],0,0.6])
    pylab.grid()
    pylab.xticks(list(pylab.xticks()[0]) + [1])
    pylab.legend(loc=4)
    pylab.xlabel("$N_{dipoles}$")
    ...
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.19/result_a.pdf')
    pylab.savefig(pp, format='pdf',dpi=fig1.dpi, bbox_inches='tight')
    pp.close()
    fig=pylab.figure(2)
    #fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.19/result_b.pdf')
    pylab.savefig(pp, format='pdf',dpi=fig1.dpi, bbox_inches='tight')
    pp.close()
    output1=zip(*output_data1)
    numpy.savetxt(r"D:\HIWI\python-script\new_new_results\4.19/4.19a.dat", output1, fmt=['%.6f']*len(output_data1))
    output2=zip(*output_data2)
    numpy.savetxt(r"D:\HIWI\python-script\new_new_results\4.19/4.19b.dat", output2, fmt=['%.6f']*len(output_data2))
