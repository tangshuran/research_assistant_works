...
if __name__ == "__main__":
    ...
    a_EUTs=[0.0,0.1,0.2693,0.5,1.0,2.0,3.0]# radius of EUT
    N_dipole = 10    # number of random dipoles
    N_obs_points=10 #number of observation points (randomly distributed) on Ring around EUT
    N_MC=20000       # number of MC runs -> average over different random configurations
    ...
    colors=cycle('bgrcmk')
    for a_EUT,clr in zip(a_EUTs,colors):   # loop frequencies, kas
        f=150*1e6
        ka=a_EUT*2*pi*f/c
        ...
        ecdfD=ECDF(Ds)
        pylab.figure(1)
        pylab.plot(deval,ecdfD(deval), '%s+-'%clr, label="ECDF (Dipoles), a=%.2f m"%(a_EUT))
    ...
    pylab.ylabel("CDF")
    pylab.title("$N_{dipoles}=%d$, MC runs=%d, $N_{obs.}=%d$, $R=%d m$"%(N_dipole,N_MC,N_obs_points,distance))
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    #pylab.show()
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\3.7/result.pdf')
