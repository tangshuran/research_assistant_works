...
if __name__ == "__main__":
    ...
    N_obs_points_list=[50,100,200,500] #number of observation points (randomly distributed) on Ring around EUT
    ...
    for N_obs_points,clr in zip(N_obs_points_list,colors):   # loop frequencies, kas
        f=300*1e6
        ka=a_EUT*2*pi*f/c
    ...
    pylab.ylabel("CDF")
    pylab.title("$N_{dipoles}=%d$, MC runs=%d, $Frequency=%d MHz$, $R=%d m$"%(N_dipole,N_MC,f/1e6,distance))
    #pylab.show()
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.1/result.pdf')
