...
if __name__ == "__main__":
    ...
    ax2.set_xlabel(r'$\stackrel{f/MHz}{ka}$')
    ax2.set_ylabel("$var(D_{max}^{R})$")
    ax2.set_title("$N_{dipoles}=%d$, MC runs=%d, $a_{EUT}=%.4f m$, $R=%d m$"%(N_dipole,N_MC,a_EUT,distance))
    #pylab.figure(1)
    ax1.plot(freqs/1e6,D_max_list, 'r+-', label="Ring")
    #pylab.figure(2)
    ax2.plot(freqs/1e6,D_max_var_list, 'r+-', label="Ring")
    ax2.set_ylim([0, 1])
    plt.show()
    ...
    ax1.plot(freqs/1e6,D_max_list, 'b+-', label="Zylinder")
    #pylab.figure(2)
    ax2.plot(freqs/1e6,D_max_var_list, 'b+-', label="Zylinder")
    ax2.legend(loc=4)
    ax1.legend(loc=4)
    plt.grid()
