...
if __name__ == "__main__":
    ...
    for mc in range(N_MC): # MC loop
        ...
        #print mc, ma, av, D
        Ds_K.append(D)
        Es2_list_max_K.append(ma)
        print "K",mc        
    Es2_ratio=numpy.divide(Es2_list_max_R,Es2_list_max_K)
    ...
    bins = np.linspace(0,1.1, 22, endpoint=True)
    ax.hist(Es2_ratio, bins,(0,1.1),facecolor='grey')
    plt.grid()
    #pylab.legend(loc=4)
    plt.xlabel(r"$\frac{E^{R^{2}}_{max}}{E^{K^{2}}_{max}}$")
