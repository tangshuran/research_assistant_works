...
if __name__ == "__main__":
    ...
    N_dipole_list = numpy.linspace(1,30,30)    # number of random dipoles
    ...
    n_listen=0
    fig1=pylab.figure(1)        
    output_data.append(N_dipole_list) 
    ...
    for N_dipole,clr in zip(N_dipole_list,colors):   # loop frequencies, kas
        ...
        for mc in range(N_MC): # MC loop
            ...
            Ds.append(D)
            Eslist.append(Es)
        Expectation_list.append(numpy.mean(Ds))
        Var_list.append(numpy.var(Ds))
       ...
       # print    
        ...
        #sys.stdout.flush()
        ecdfD=ECDF(Ds)
    Expectation_array=numpy.array(Expectation_list)
    Var_array=numpy.array(Var_list)
    p_array=numpy.divide(Expectation_array,Var_array)
    b_array=numpy.divide(Expectation_array*Expectation_array,Var_array)
    ...
    pylab.figure(1)
    pylab.plot(N_dipole_list,p_array, '%s+-'%"y",label="$p(D_{max}^{R})$")
    output_data.append(p_array)
    pylab.plot(N_dipole_list,b_array, '%s+-'%"r",label="$b(D_{max}^{R})$")
    output_data.append(b_array)
    pylab.axis([N_dipole_list[0],N_dipole_list[-1],0,30])
    pylab.grid()
    pylab.legend(loc=4)
    pylab.xlabel("$N_{dipole}$")
    pylab.ylabel("Parameter of gamma distribution")
    pylab.title("$f=%d GHz$, MC runs=%d, $a_{EUT}=%.2f m$, $R=%d m$"%(f/1e9,N_MC,a_EUT,distance))
    #pylab.show()
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(18.5, 10.5)
    pp = PdfPages(r'D:\HIWI\python-script\new_new_results\4.23/result_b.pdf')
    plt.savefig(pp, format='pdf',dpi=fig1.dpi, bbox_inches='tight')
    pp.close()
    output=zip(*output_data)
    numpy.savetxt(r"D:\HIWI\python-script\new_new_results\4.23/4.23b.dat", output, fmt=['%.6f']*len(output_data))
