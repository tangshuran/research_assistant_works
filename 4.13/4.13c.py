...
if __name__ == "__main__":
    ...
    for N_MC,clr in zip(N_MC_list,colors):   # loop frequencies, kas
        ...
        ecdfD=ECDF(Ds)
        pylab.figure(1)
        pylab.plot(deval,ecdfD(deval), '%s+-'%clr, label="ECDF (Dipoles),$N_{MCS}$=%d"%(N_MC))
    ...
    pylab.ylabel("CDF")
    pylab.title("$N_{dipoles}=%d$, $N_{obs}=%d$, $R=%d m$, $a_{EUT}$=%.2f m, f=%d GHz"%(N_dipole,N_obs_points,distance,a_EUT,f/1e9))
    #pylab.show()
    ...
    numpy.savetxt(r"D:\HIWI\python-script\new_new_results\4.13/4.13c.dat", output, fmt=['%.6f']*len(output_data))
