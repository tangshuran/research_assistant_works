...
if __name__ == "__main__":
    ...
    freqs=numpy.array([1500])*1e6#[30,50,80,100,150, 200,250, 300,350, 400,450, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500])*1e6#numpy.array(range(30,301,30))*1000000#numpy.logspace(10,11,3)  # generate frequencies
    ...
    ax.set_title(r"$f=%d Mhz,N_{dipoles}=%d$, MC runs=%d, $N_{obs}=%d$, $R=%d m$,$a_{EUT}$=%.2f m,\varphi_{max}=%.1f^{\circ}$"%(f/1e6,N_dipole,N_MC,N_obs_points,distance,a_EUT,degrees(max_angle)))
    #pylab.legend(loc=4)
